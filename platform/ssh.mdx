...
Encrova can be used to issue SSH credentials to clients to provide short-lived, secure SSH access to infrastructure;
...
The following concepts are useful to know when working with Encrova SSH:
...
When using Encrova SSH to provision client access to a remote host, an operator must create a SSH CA in Encrova; a certificate template under it,
specifying policies such as allowed users that can be requested under that template by a client; and configure the host to trust certificates issued by the Encrova SSH CA.

When a client needs access to a host, they authenticate with Encrova and request a SSH certificate (and optionally key pair)
...
sequenceDiagram
  ...
  participant Encrova as Encrova (SSH CA)
  participant Host as Remote Host

  Note over Client,Client: Step 1: Client Authentication with Encrova
  Client->>Encrova: Send credential(s) to authenticate with Encrova

  Encrova-->>Client: Return access token

  Note over Client,Encrova: Step 2: SSH Certificate Request
  Client->>Encrova: Make authenticated request for SSH certificate via either /api/v1/ssh/issue or /api/v1/ssh/sign

  Encrova-->>Client: Return signed SSH certificate (and optionally key pair)
...
At a high-level, Encrova issues a signed SSH certificate to a client that can be used to access a remote host.

To be more specific:

1. The client authenticates with Encrova; this can be done using a machine identity [authentication method](/documentation/platform/identities/machine-identities) or a user [authentication method](/documentation/platform/identities/user-identities).
2. The client makes an authenticated request for an SSH certificate via either the `/api/v1/ssh/issue` or `/api/v1/ssh/sign` endpoints. Note that if the client wishes to use an existing SSH key pair, it can use the `/api/v1/ssh/sign` endpoint; otherwise, it can use the `/api/v1/ssh/issue` endpoint to have Encrova issue a new SSH key pair in conjunction with the certificate.
...
## Guide to Configuring Encrova SSH

In the following steps, we explore how to configure Encrova SSH to start issuing SSH certificates to clients as well as a remote host to trust these certificates
as part of the SSH operation.

<Steps>
  <Step title="Configuring Encrova SSH">
    1.1. Start by creating a SSH project in the SSH tab of your organization.
    <Frame>
        <img src="/images/ssh-project.png"/>
    </Frame>
    
    1.2. Next, create a CA in the **Certificate Authorities** tab of the
    project.
      <Frame>
          <img src="/images/ssh-create-ca-1.png"/>
      </Frame>
      <Frame>
          <img src="/images/ssh-create-ca-2.png"/>
      </Frame>
    ...
    With certificate templates, you can specify, for example, that certificates issued under a template are only allowed for users with a specific username like `ec2-user` or perhaps that the max TTL requested cannot exceed 1 year.
      <Frame>
          <img src="/images/ssh-create-template-1.png"/>
      </Frame>
      <Frame>
          <img src="/images/ssh-create-template-2.png"/>
      </Frame>
  ...
  <Step title="Configuring the remote host">
    ...
    2.1. Begin by downloading the CA's public key from the CA's details section.
      <Frame>
          <img src="/images/ssh-ca-public-key.png"/>
      </Frame>
    ...
    At this point, the remote host is configured to trust SSH certificates issued by the Encrova SSH CA.

  </Step>
</Steps>

## Guide to Using Encrova SSH to Access a Host
...
<Steps>
  <Step title="Authenticate with Encrova">
...
```
  ...
  <Step title="Obtain a SSH certificate and load it into the SSH agent">
    Run the `encrova ssh issue-credentials` command, specifying the `--addToAgent` flag to automatically load the SSH certificate into the SSH agent.
    ```bash
    encrova ssh issue-credentials --certificateTemplateId=<certificate-template-id> --principals=<username> --addToAgent
    ```
    ...
    For fuller documentation on commands and flags supported by the Encrova CLI for SSH, refer to the docs [here](/cli/commands/ssh).
